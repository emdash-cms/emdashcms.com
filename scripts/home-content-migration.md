...
2. Confirm that each preserved block's `beforeHash` and `afterHash` match, including the Hero. Copy `revision` and `planHash` from the output. The command refuses missing, duplicate, or reordered marketing blocks, an existing unpublished draft, and a scheduled publication.
